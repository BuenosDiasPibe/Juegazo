...
namespace Juegazo
{
  ...
  public class Entity : Sprite
  {
    ...
    public Entity(Texture2D texture, Rectangle sourceRectangle, Rectangle Destrectangle, Rectangle collider, Color color) 
      : base(texture, sourceRectangle, Destrectangle, color)
    {
      velocity = new();
      onGround = false;
      maxHealth = 1;
    ...
    public Entity( Texture2D texture, Rectangle sourceRectangle, Rectangle Destrectangle, float collider, Color color)
      : base(texture, sourceRectangle, Destrectangle, color)
    {
      velocity = new();
      onGround = false;
      maxHealth = 1;
      health = maxHealth;
      this.collider = new(Destinationrectangle.X, Destinationrectangle.Y,
          (int)(Destinationrectangle.Width * collider), (int)(Destinationrectangle.Height * collider)
      );
    }

    public Entity( Texture2D texture, Rectangle sourceRectangle, Rectangle Destrectangle, List<Component> componentList, Color color) 
      : base(texture, sourceRectangle, Destrectangle, color)
    {
      velocity = new();
      onGround = false;
      maxHealth = 1;
      health = maxHealth;
      collider = new( Destinationrectangle.X, Destinationrectangle.Y,
            (int)(Destinationrectangle.Width * 0.8), (int)(Destinationrectangle.Height * 0.8)
      );
      ...
      : base(texture, sourceRectangle, Destrectangle, color)
    {
      ...
      this.collider = new(
          (int)(Destinationrectangle.X + WidthCollider * 0.5),
          (int)(Destinationrectangle.Y * WidthCollider * 0.5),
          WidthCollider,
          HeightCollider
      );
      foreach (Component component in componentList)
    ...
    public virtual void Draw(GameTime gameTime, SpriteBatch spriteBatch)
    {
      ...
      if (!componentDrew) //i swear to god it took a long time of trial and error, fuck this shit
    ...
    public Component GetComponent(Type type) =>
      componentDictionary.TryGetValue(type, out Component component) ? component : null;

    public Component GetComponent<T>()
      where T : Component =>
        componentDictionary.TryGetValue(typeof(T), out Component component) ? component : null;

    public bool hasComponent(Type type) => componentDictionary.ContainsKey(type);

    public bool hasComponent<T>()
      where T : Component => componentDictionary.ContainsKey(typeof(T));

    public Component RemoveComponent<T>()
      where T : Component => RemoveComponent(typeof(T));
